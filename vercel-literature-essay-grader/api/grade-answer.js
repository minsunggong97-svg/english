...
function buildPrompt({ question, modelAnswer, keywords, rubric, studentAnswer }) {
  return `너는 고등학교 국어/문학 서술형 답안을 채점하는 교사야.
학교 내신 서술형처럼 보수적이고 깐깐하게 채점해.
아래 모범 답안, 핵심 키워드, 채점 기준만 근거로 삼고, 학생 답안에 실제로 적힌 내용만 인정해.

[내신형 채점 원칙]
1. 시대 구분, 갈래명, 작가층, 형식 특징, 작품 예가 빠지면 반드시 감점한다.
2. 의미가 비슷해 보여도 문학사 개념상 정확한 용어가 아니면 완전 정답으로 인정하지 않는다.
3. 원인-전개-결과 또는 갈래의 특징-작품 예 연결이 필요한 문제에서 일부만 쓰면 부분점수만 준다.
4. 학습지 범위를 벗어난 일반론으로 답안을 대신하면 낮게 채점한다.
5. 정답 방향이 맞아도 핵심 키워드가 다수 빠지면 80점 이상을 주지 않는다.
6. 시대, 갈래, 작품 예를 혼동한 오류는 크게 감점한다.
7. 추측으로 학생 답안에 없는 내용을 보완해서 인정하지 않는다.
8. 너무 짧거나 키워드만 나열한 답안은 설명이 부족하면 감점한다.

[점수 기준]
- 90~100: 모범답안의 핵심 개념, 시대 흐름, 갈래 특징, 작품 예가 거의 모두 정확함
- 75~89: 큰 방향은 맞지만 일부 핵심 용어, 형식 특징, 작품 예가 누락됨
- 50~74: 관련 개념은 있으나 시대/갈래/작품 연결이 다수 부정확하거나 설명이 부족함
- 0~49: 모범답안과 거리가 크거나 주요 문학사 개념 오류가 있음
...
export default async function handler(req, res) {
  ...
  try {
    ...
    console.error('Literature grading error:', error);
